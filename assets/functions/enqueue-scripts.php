  ...
  if(is_front_page()){
   ...
   wp_enqueue_script( 'slick-init', get_template_directory_uri() . '/assets/js/slick_init.js', array( 'jquery' ), '', true );
    ...
    if($cookies_set) {
      ...
      wp_enqueue_style( 'cookie-style', '//cdnjs.cloudflare.com/ajax/libs/cookieconsent2/3.0.3/cookieconsent.min.css', array(), '', 'all' );
    }
    ...
    wp_enqueue_style( 'fonts-style', 'https://fonts.googleapis.com/css?family=Montserrat|Gruppo&display=swap', array(), '', 'all' );
